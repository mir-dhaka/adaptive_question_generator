...
export class PlantationComponent {
        ...
        this.service.getAll(this.slug, (res) => {
            if (res.success == true) {
                this.items = res.data.filter((x) => x.status != -1).map(item => {
                    item.plantationDate = this.formatDate(item.plantationDate);
                    return item;
                });
                if (showToast) {
    ...
    editItem(item: any) {
        this.update = true;
        this.item = { ...item };
        this.item.plantationDate = new Date(this.item.plantationDate)
        this.itemDialog = true;
